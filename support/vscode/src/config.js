    ...
    CONF["replace.userRules"].forEach(rule => {
      ...
      if (rule && rule.name && rule.re && rule.sub) {
        try {
          ...
          replace.res.push({
            name: rule.name, re: new RegExp(pattern, flags),
            sub: rule.sub, default: true
          })
        } catch (e) {
          valid = false
        }
...
function userColorCheck() {
  if (CONF["unify.userColors"].length) {
    // colors provided
    CONF["unify.userColors"].forEach(c => {
      let valid = true
      if (c && c.name && c.color) {
        if (Color(c.color).isValid()) unify.userColor.push({ name: c.name, color: c.color })
        else valid = false
      } else valid = false
...
function reOpts() { // + check
  ...
  CONF["replace.disabledRules"].forEach(rule => {
    if (replace.res.some(re => re.name === rule)) object[rule] = false
    else vscode.window.showWarningMessage(`Rule "${rule}" does not exist!`)
  })
  return object
}
...
let confChange = vscode.workspace.onDidChangeConfiguration(event => {
  if (Object.keys(CONF).some(setting => event.affectsConfiguration(`jotdoc.${setting}`))) {
    vscode.window
      .showInformationMessage("Code must reload to reflect changes.", "Reload")
      .then(selection => {
...
module.exports = { enabledFeatures, confChange }
